...
export function ScoringForm({ onSubmit, data, isSelected, judgeID }) {
  ...
  const handleFormSubmit = handleSubmit((formValues) => {
    if (data.IsUni === false) {
      ...
      setSubmittedItems((prev) => {
        ...
        newSet.add(data.RegisterPlayGenresID)
        return newSet
      })
    }
  })
  ...
  console.log('data.is', hasExistingPoints)

  useEffect(() => {
    console.log('Danh sách đã lưu:', submittedItems)
  }, [submittedItems])

  const isChecked =
    hasExistingPoints || submittedItems.has(data.RegisterPlayGenresID)

  return data.IsUni === true ? (
    <Stack
      direction="row"
      alignItems="center"
      component="form"
    ...
    >
      ...
      >
        <Typography variant="h5" color="white" fontWeight={600}>
          ...
          {/* {hasPoints ? <CheckIcon sx={{ fontSize: 64 }} /> : 'SAVE'} */}
          {isChecked ? <CheckIcon sx={{ fontSize: 64 }} /> : 'SAVE'}
        </Typography>
      </Button>
    </Stack>
  ) : (
    ...
    >
      ...
      >
        <Typography variant="h5" color="white" fontWeight={600}>
          {data?.PointPlay ? <CheckIcon sx={{ fontSize: 64 }} /> : 'SAVE'}
        </Typography>
      </Button>
    </Stack>
  )
